...
class TestSessionAffinity:
    """Tests for session routing unique to PrefixCacheAffinityRouter.

    Basic SessionMixin behavior (sticky routing, fallback on replica removal,
    cleanup on death, multiplex priority) is tested in
    ray/serve/tests/unit/test_pow_2_request_router.py::TestSessionAffinity.
    These tests cover the interaction between session affinity and prefix cache.
    """
    ...
    async def test_session_takes_priority_over_prefix_cache(
        ...
        """Session affinity should override prefix cache optimization."""
